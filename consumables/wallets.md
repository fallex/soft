...
Платный, но удобный для больших ферм.

## Способ 3 — Из других источников

* Импорт из старых ферм/софтов (если уже есть пул).
* Restoration из MetaMask/etc. с разными деривациями (тот же seed → разные адреса).
...
2. Залить газ с прогревного кошелька (см. [Безопасность → Аккаунты](../security/wallets.md)).
...
Для Outcome-MXW на Arbitrum Sepolia — копейки (тестнет, газ почти 0).

## Чек-лист

* [ ] Сгенерены приватники под отдельный софт
* [ ] Адреса вынесены в отдельный файл (для пополнения)
* [ ] Газ залит на адреса
* [ ] Приватники отправлены в зашифрованную БД софта
* [ ] Файл `wallets.txt` (или подобный) удалён после переноса
